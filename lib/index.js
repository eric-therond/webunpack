...
function installWebPack(tmpwebpack) {
    ...
    try {
        execSync('cd '+tmpwebpack+';rm -rf node_modules/ 2>/dev/null');
        execSync('cd '+tmpwebpack+';npm install --prefix '+tmpwebpack+' --save -f --prefer-offline --no-audit 2>/dev/null');
        execSync('cd '+tmpwebpack+';npx webpack 2>/dev/null');
    }
    catch(e) {
        //console.log("error = "+e);
        return false;
...
function unpackString(packeddata, signaturedb, onlythemostsimilitudesmodule = false) {
...
function unpackFile(packedfile, signaturedb, onlythemostsimilitudesmodule = false) {
